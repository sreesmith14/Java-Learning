...
public class EmpStream {
	...
	public static void main(String[] args) {
		Employee[] emparr = { new Employee("Sachin", "Tendulkar", "IT", 50000),
				new Employee("Amir", "Khan", "HR", 45000), new Employee("MS", "Dhoni", "Sales", 60000),
				new Employee("Virat", "Koli", "Sales", 55000), new Employee("Yuvaraj", "Singh", "IT", 35000) };
		...
		empList.stream().filter(p1).sorted(Comparator.comparing(Employee::getFname)).forEach(System.out::println);

		System.out.println(empList.stream().filter(p1).findFirst().get());

		/*
		 * IT sac amir Sales MSD VK HR Amir
		 */
		System.out.println("-----------------------------------------------------------");
